...
class DataProcessor:
    ...
    def process_list(self, data_list):
        """Обработка списков - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        if isinstance(data_list, list):
            result = {
                'original': data_list,
                'length': len(data_list),
                'unique_items': list(set(data_list)) if data_list else [],
                'type': 'list'
            }

            # Сортировка только если все элементы одного типа и список не пустой
            if data_list and all(isinstance(x, (int, float, str)) for x in data_list):
                try:
                    result['sorted'] = sorted(data_list)
                except TypeError:
                    result['sorted'] = None
            else:
                result['sorted'] = None

            # Числовая статистика только для числовых списков и не пустых
    ...
    def auto_detect_and_process(self, data):
        """Автоматическое определение типа данных и их обработка - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        # Проверяем boolean в первую очередь, так как bool является подклассом int
...
class TestDataProcessor(unittest.TestCase):
    ...
    def test_process_empty_list(self):
        """TC-009: Тест обработки пустого списка - ИСПРАВЛЕН"""
        result = self.processor.process_list([])

        self.assertEqual(result['length'], 0)
        self.assertEqual(result['unique_items'], [])
        # Проверяем, что числовая статистика отсутствует
    ...
    def test_process_mixed_list(self):
        """TC-010: Тест обработки смешанного списка - ИСПРАВЛЕН"""
        ...
        self.assertNotIn('min', result)
        self.assertNotIn('max', result)
        # Проверяем, что сортировка не применялась (разные типы)
        self.assertIsNone(result.get('sorted'))
        self.assertEqual(result['type'], 'list')
    ...
    def test_auto_detect_boolean(self):
        """Тест автоопределения логического значения - ИСПРАВЛЕН"""
        result = self.processor.auto_detect_and_process(True)
        self.assertEqual(result['type'], 'boolean')
        self.assertTrue(result['original'])
...
class TestIntegration(unittest.TestCase):
    ...
    def test_complex_workflow(self):
        """Тест сложного рабочего процесса - ИСПРАВЛЕН"""
        ...
        self.assertEqual(results[4]['type'], 'boolean')  # True должно быть boolean
